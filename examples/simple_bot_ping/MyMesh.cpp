    ...
    if (current_channel == bot_channel_idx) {
      ...
      if (strstr(_text, BOT_NAME_PLAIN) != nullptr) {
        ...
        if (strstr(_text, "репитеры") != nullptr || strstr(_text, "repeaters") != nullptr) {
          ...
          for (int i = 0; i < _stats.num_repeaters; i++) {
            if (strstr(_stats.repeaters[i].name, "UnknownRepeater") == nullptr) {
...
void MyMesh::begin(FILESYSTEM &fs) {
  ...
  addChannel("channel2", GROUP_2_PSK);
  addChannel("channel3", GROUP_3_PSK);
  addChannel("channel4", GROUP_4_PSK);
  addChannel("channel5", GROUP_5_PSK);
  addChannel("channel6", GROUP_6_PSK);
  addChannel("channel7", GROUP_7_PSK);
  addChannel("channel8", GROUP_8_PSK);
  addChannel("channel9", GROUP_9_PSK);
  addChannel("channel10", GROUP_10_PSK);
...
void MyMesh::handleCommand(const char *command) {
  ...
  if (memcmp(command, "post ", 5) == 0) {
    // send GroupChannel msg
    sendMessage(&command[5], bot_channel->channel, PATH_HASH_MODE);
  ...
  } else if (memcmp(command, "remove ", 7) == 0 && strlen(command) >= 7 + PATH_HASH_MODE * 2) {
    char _prefix[PATH_HASH_MODE * 2 + 1]{};
    uint8_t prefix[PATH_HASH_MODE]{};
    for (int i = 0; i < PATH_HASH_MODE * 2; i++) {
      if (mesh::Utils::isHexChar(command[i + 7])) {
        if (command[i + 7] >= 'a' && command[i + 7] <= 'f') {
          _prefix[i] = command[i + 7] - 32;
        } else {
          _prefix[i] = command[i + 7];
        }
      }
    }
    if (_prefix[0] != 0) {
